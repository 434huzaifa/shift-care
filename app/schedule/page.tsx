    ...
    <div className="p-6 w-full h-screen flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <Select
            instanceId="schedule-month-select"
          ...
          <Select
            instanceId="schedule-year-select"
            options={years.reverse().map(y => ({ value: y, label: y.toString() }))}
              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Display shifts only for current month days */}
                    {isCurrentMonth && (
                      <div className="space-y-1 text-xs">
                        {dayShifts.slice(0, 3).map((shift, idx) => {
                          const colors = [
                            { bg: 'bg-blue-100 dark:bg-blue-900', text: 'text-blue-900 dark:text-blue-100' },
                            { bg: 'bg-green-100 dark:bg-green-900', text: 'text-green-900 dark:text-green-100' },
                            { bg: 'bg-purple-100 dark:bg-purple-900', text: 'text-purple-900 dark:text-purple-100' },
                            { bg: 'bg-orange-100 dark:bg-orange-900', text: 'text-orange-900 dark:text-orange-100' },
                            { bg: 'bg-pink-100 dark:bg-pink-900', text: 'text-pink-900 dark:text-pink-100' },
                            { bg: 'bg-indigo-100 dark:bg-indigo-900', text: 'text-indigo-900 dark:text-indigo-100' },
                          ];
                          const colorSet = colors[shift.id % colors.length];
                          
                          return (
                            <div
                              key={`${shift.id}-${shift.occurrenceDate}-${idx}`}
                              className={`${colorSet.bg} ${colorSet.text} px-1.5 py-0.5 rounded text-xs truncate`}
